...
from .fetchers import fetch_sources, load_demo_items, load_items_file
...
def run_pipeline(
    ...
    *,
    ...
    warnings: list[str] = []
    if demo_items_path and items_path:
        raise RuntimeError("Use either demo items or a local items file, not both.")
    if demo_items_path:
        items = load_demo_items(demo_items_path)
    elif items_path:
        items = load_items_file(items_path)
    else:
