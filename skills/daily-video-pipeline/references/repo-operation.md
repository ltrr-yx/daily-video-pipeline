...
Use this when the user already collected the day's material outside RSS/Atom. The JSON shape matches `examples/demo_items.json`: `title`, `url`, `source_name`, `published_at`, `summary`, and optional `tags`.
...
Run the publishing gate:

```bash
daily-video review --output outputs/YYYY-MM-DD/project-name
```

It writes `review_report.md` and checks required artifacts, media metadata, private terms, stale draft files, and known TTS-prone polyphonic words in output text.
...
3. Use `--items local_items.json` for structured local material. If they want finished-script rendering, propose adding `render --script script.md`.
